...
int main(void)
{
	  ...
	  HAL_TIM_Base_Start(&htim2);			//counter every 1 us

	  /*start interrupt on receiving the data*/
	  HAL_UART_Receive_IT(&huart1 , &rx_byte, 1);



	  EV_Init(&ev);  //ev initaliization
	  HCSR04_Init();  //hsrc init
	  ADAS_Init(&adas);// initalize left right ans centre
	  Fault_Init(&flt);
	  ...
	  while (1)
	  {
	      /* ── EV update every 10ms via TIM1 flag ─────────────────── */
	      if (flag_ev) {
	          flag_ev = 0;
	      ...
	      if(sensor_flag == 1)   //100ms tick
	      {
	    	  ...
	    	  // update TTC alarm blind spot detection
	    	  ADAS_Update(&adas, &ev);

	    	  /*validating mototr temmp soc*/
	    	  Fault_Check(&flt , &ev, &adas);

	    	  // waiting to rx the data and process cmd
	    	  Shell_Process();
...
/* uart rx byte - push to shell ring buffer, re-are*/
